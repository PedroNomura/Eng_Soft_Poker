...
class player:
    def __init__(self,name,chips,cards):
        self.name = name
        self.fichas = chips
        self.cards = cards # ?
...
class PokerRoom:
    ...
    def start(self): # comeca a logica da partida, ditribui as cartas, marca o BB, para cada player começando pelo player depois BB 
        # distribui as cartas para todos os players TESTE
        for player in players:
            player.cards = requests.get("https://deckofcardsapi.com/api/deck/{deck_id}/draw/?count=2").json()['cards']
        
    def adicionar_jogador(self, name, ):
